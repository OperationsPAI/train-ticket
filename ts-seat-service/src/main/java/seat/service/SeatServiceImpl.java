...
public class SeatServiceImpl implements SeatService {
  ...
  public Response distributeSeat(Seat seatRequest, HttpHeaders headers) {
    ...
    if (trainNumber.startsWith("G") || trainNumber.startsWith("D")) {
      ...
      // Call the microservice to query for residual Ticket information: the set of the Ticket sold
      // for the specified seat type
      ...
      SeatServiceImpl.LOGGER.info(
          "[distributeSeat][Left ticket info][info is : {}]", re3.getBody().toString());
      leftTicketInfo = re3.getBody().getData();
    } else {
      SeatServiceImpl.LOGGER.info(
          "[distributeSeat][TrainNumber start][Other Capital Except D and G]");
      // Call the microservice to query for residual Ticket information: the set of the Ticket sold
      // for the specified seat type
      ...
      re3 =
          restTemplate.exchange(
              orderOtherServiceUrl + "/api/v1/orderOtherService/orderOther/tickets",
              HttpMethod.POST,
              requestEntity,
              new ParameterizedTypeReference<Response<LeftTicketInfo>>() {});
      SeatServiceImpl.LOGGER.info(
    ...
    if (leftTicketInfo != null) {
      ...
      for (Ticket soldTicket : soldTickets) {
        ...
        // Tickets can be allocated if the sold ticket's end station before the start station of the
        // request
        if (stationList.indexOf(soldTicketDestStation) < stationList.indexOf(startStation)) {
          ticket.setSeatNo(soldTicket.getSeatNo());
          SeatServiceImpl.LOGGER.info(
              "[distributeSeat][Assign new tickets][Use the previous distributed seat number][seat"
                  + " number:{}]",
              soldTicket.getSeatNo());
    ...
    SeatServiceImpl.LOGGER.info(
        "[distributeSeat][Assign new tickets][Use a new seat number][seat number:{}]", seat);
  ...
  public Response getLeftTicketOfInterval(Seat seatRequest, HttpHeaders headers) {
    ...
    String trainNumber = seatRequest.getTrainNumber();
    SeatServiceImpl.LOGGER.info(
        "[getLeftTicketOfInterval][Seat request][request:{}]", seatRequest.toString());
    if (trainNumber.startsWith("G") || trainNumber.startsWith("D")) {
      SeatServiceImpl.LOGGER.info(
          "[getLeftTicketOfInterval][TrainNumber start with G|D][trainNumber:{}]", trainNumber);
      ...
      SeatServiceImpl.LOGGER.info(
          "[getLeftTicketOfInterval][Get Order tickets result][result is {}]", re3);
      leftTicketInfo = re3.getBody().getData();
    } else {
      SeatServiceImpl.LOGGER.info(
          "[getLeftTicketOfInterval][TrainNumber start with other capital][trainNumber:{}]",
          trainNumber);
      // Call the micro service to query all the station information for the trains
      HttpEntity requestEntity = new HttpEntity(headers);
      // Call the micro service to query for residual Ticket information: the set of the Ticket sold
      // for the specified seat type
      requestEntity = new HttpEntity(seatRequest, headers);
      String orderOtherServiceUrl = getServiceUrl("ts-order-other-service");
      re3 =
    ...
    if (leftTicketInfo != null) {
      ...
      for (Ticket soldTicket : soldTickets) {
        ...
        if (stationList.indexOf(soldTicketDestStation) < stationList.indexOf(startStation)) {
          SeatServiceImpl.LOGGER.info(
              "[getLeftTicketOfInterval][Ticket available or sold][The previous distributed seat"
                  + " number is usable][{}]",
              soldTicket.getSeatNo());
          numOfLeftTicket++;
        }
  ...
  private double getDirectProportion(HttpHeaders headers) {
    ...
    ResponseEntity<Response<Config>> re =
        restTemplate.exchange(
            configServiceUrl + "/api/v1/configservice/configs/" + configName,
            HttpMethod.GET,
            requestEntity,
            new ParameterizedTypeReference<Response<Config>>() {});
    Response<Config> configValue = re.getBody();
    SeatServiceImpl.LOGGER.info(
        "[getDirectProportion][Configs is : {}]", configValue.getData().toString());
