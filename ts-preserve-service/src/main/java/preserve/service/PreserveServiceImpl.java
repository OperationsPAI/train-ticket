...
public class PreserveServiceImpl implements PreserveService {
  ...
  public Response preserve(OrderTicketsInfo oti, HttpHeaders headers) {
    ...
    if (response.getStatus() == 0) {
      ...
      if (oti.getSeatType() == SeatClass.FIRSTCLASS.getCode()) {
        if (tripResponse.getConfortClass() == 0) {
          PreserveServiceImpl.LOGGER.warn(
    ...
    if (oti.getAssurance() == 0) {
      ...
      Response addAssuranceResult =
          addAssuranceForOrder(oti.getAssurance(), cor.getData().getId().toString(), headers);
    ...
    if (oti.getFoodType() != 0) {
      ...
      if (oti.getFoodType() == 2) {
        ...
        // PreserveServiceImpl.LOGGER.info("foodstore= {}   {}   {}", foodOrder.getFoodType(),
        // foodOrder.getStationName(), foodOrder.getStoreName());
      }
  ...
  private Response addAssuranceForOrder(
    ...
    ResponseEntity<Response> reAddAssuranceResult =
        restTemplate.exchange(
            assuranceServiceUrl
  ...
  private Response<TripAllDetail> getTripAllDetailInformation(
    ...
    ResponseEntity<Response<TripAllDetail>> reGetTripAllDetailResult =
        restTemplate.exchange(
            travelServiceUrl + "/api/v1/travelservice/trip_detail",
            HttpMethod.POST,
