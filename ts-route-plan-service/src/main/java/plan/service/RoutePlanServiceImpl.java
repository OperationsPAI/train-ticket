...
public class RoutePlanServiceImpl implements RoutePlanService {
  ...
  public Response searchMinStopStations(RoutePlanInfo info, HttpHeaders headers) {
    ...
    RoutePlanServiceImpl.LOGGER.info(
        "[searchMinStopStations][Start and Finish][From Id: {} To: {}]",
        fromStationId,
        toStationId);
    ...
    ResponseEntity<Response<ArrayList<Route>>> re =
        restTemplate.exchange(
            routeServiceUrl
                + "/api/v1/routeservice/routes/"
                + info.getStartStation()
                + "/"
                + info.getEndStation(),
            HttpMethod.GET,
            requestEntity,
    ...
    RoutePlanServiceImpl.LOGGER.info(
        "[searchMinStopStations][Get the route][Candidate Route Number: {}]", routeList.size());
    ...
    ResponseEntity<Response<ArrayList<ArrayList<Trip>>>> re2 =
        restTemplate.exchange(
            travelServiceUrl + "/api/v1/travelservice/trips/routes",
            HttpMethod.POST,
            requestEntity,
    ...
    re2 =
        restTemplate.exchange(
            travel2ServiceUrl + "/api/v1/travel2service/trips/routes",
            HttpMethod.POST,
            requestEntity,
            new ParameterizedTypeReference<Response<ArrayList<ArrayList<Trip>>>>() {});
    ...
    RoutePlanServiceImpl.LOGGER.info(
        "[searchMinStopStations][Get train Information][Trips Num: {}]", finalTripResult.size());
    ...
    for (Trip trip : trips) {
      ...
      String requestUrl = "";
      if (trip.getTripId().toString().charAt(0) == 'D'
          || trip.getTripId().toString().charAt(0) == 'G') {
    ...
    RoutePlanServiceImpl.LOGGER.info(
        "[searchMinStopStations][Trips Response Unit Num: {}]", tripResponses.size());
  ...
  private Route getRouteByRouteId(String routeId, HttpHeaders headers) {
    ...
    if (result.getStatus() == 0) {
      RoutePlanServiceImpl.LOGGER.error(
          "[getRouteByRouteId][Get Route By Id Fail][RouteId: {}]", routeId);
      return null;
    } else {
  ...
  private ArrayList<TripResponse> getTripFromHighSpeedTravelServive(
    ...
    RoutePlanServiceImpl.LOGGER.info(
        "[getTripFromHighSpeedTravelServive][Route Plan Get Trip][Size:{}]", tripResponses.size());
    return tripResponses;
  }

  private ArrayList<TripResponse> getTripFromNormalTrainTravelService(
      TripInfo info, HttpHeaders headers) {
    HttpEntity requestEntity = new HttpEntity(info, headers);
    String travel2ServiceUrl = getServiceUrl("ts-travel2-service");
    ResponseEntity<Response<ArrayList<TripResponse>>> re =
    ...
    RoutePlanServiceImpl.LOGGER.info(
        "[getTripFromNormalTrainTravelService][Route Plan Get TripOther][Size:{}]", list.size());
    return list;
  }
  ...
  private List<String> getStationList(String tripId, HttpHeaders headers) {
    ...
    if (tripId.charAt(0) == 'G' || tripId.charAt(0) == 'D') {
      path = travelServiceUrl + "/api/v1/travelservice/routes/" + tripId;
    } else {
      path = travel2ServiceUrl + "/api/v1/travel2service/routes/" + tripId;
    }
    HttpEntity requestEntity = new HttpEntity(headers);
    ResponseEntity<Response<Route>> re =
        restTemplate.exchange(
