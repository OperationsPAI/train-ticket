...
public class StationServiceImpl implements StationService {
  ...
  public Response create(Station station, HttpHeaders headers) {
    ...
    StationServiceImpl.LOGGER.error(
        "[create][Create station error][Already exists][StationId: {}]", station.getId());
  ...
  public Response update(Station info, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      StationServiceImpl.LOGGER.error(
          "[update][Update station error][Station not found][StationId: {}]", info.getId());
  ...
  public Response delete(String stationsId, HttpHeaders headers) {
    ...
    StationServiceImpl.LOGGER.error(
        "[delete][Delete station error][Station not found][StationId: {}]", stationsId);
    return new Response<>(0, "Station not exist", null);
  }
  ...
  public Response query(HttpHeaders headers) {
    ...
    if (stations != null && !stations.isEmpty()) {
      ...
      StationServiceImpl.LOGGER.warn(
          "[query][Query stations warn][Find all stations: {}]", "No content");
  ...
  public Response queryForId(String stationName, HttpHeaders headers) {
    ...
    if (station != null) {
      ...
      StationServiceImpl.LOGGER.warn(
          "[queryForId][Find station id warn][Station not found][StationName: {}]", stationName);
  ...
  public Response queryForIdBatch(List<String> nameList, HttpHeaders headers) {
    ...
    if (!result.isEmpty()) {
      ...
      StationServiceImpl.LOGGER.warn(
          "[queryForIdBatch][Find station ids warn][Stations not found][StationNameNumber: {}]",
          nameList.size());
  ...
  public Response queryById(String stationId, HttpHeaders headers) {
    ...
    if (station.isPresent()) {
      ...
      StationServiceImpl.LOGGER.error(
          "[queryById][Find station name error][Station not found][StationId: {}]", stationId);
  ...
  public Response queryByIdBatch(List<String> idList, HttpHeaders headers) {
    ...
    if (!result.isEmpty()) {
      ...
      StationServiceImpl.LOGGER.error(
          "[queryByIdBatch][Find station names error][Stations not found][StationIdNumber: {}]",
          idList.size());
