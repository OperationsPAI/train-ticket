...
public class AdminRouteServiceImpl implements AdminRouteService {
  ...
  public Response getAllRoutes(HttpHeaders headers) {
    ...
    ResponseEntity<Response> re =
        restTemplate.exchange(
            routeServiceUrl + "/api/v1/routeservice/routes",
            HttpMethod.GET,
            requestEntity,
            Response.class);
    if (re.getStatusCode() != HttpStatus.ACCEPTED) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "[getAllRoutes][receive response][Get routes error][response code: {}]",
  ...
  public Response createAndModifyRoute(RouteInfo request, HttpHeaders headers) {
    ...
    if (re.getStatusCode() != HttpStatus.ACCEPTED) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "[createAndModifyRoute][receive response][Get status error][response code: {}]",
            re.getStatusCodeValue());
      }
    }
    return re.getBody();
  ...
  public Response deleteRoute(String routeId, HttpHeaders headers) {
    ...
    if (re.getStatusCode() != HttpStatus.ACCEPTED) {
      if (logger.isErrorEnabled()) {
        logger.error(
            "[deleteRoute][response response][Delete error][response code: {}]",
            re.getStatusCodeValue());
      }
    }
    return re.getBody();
