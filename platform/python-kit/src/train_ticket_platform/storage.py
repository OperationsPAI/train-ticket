...
class OutboxRelay:
    ...
    def _cleanup(self) -> None:
        ...
        Deletes in BATCHES, and reports what it did. The previous version issued
        one unbounded DELETE per table inside ``except Exception: pass``, and on a
        long-running cluster that combination silently stopped working:
        journey-order's ``processed_events`` had reached 1,092,388 rows and 205 MB,
        with 1,085,529 of them past the 5-minute retention and the oldest 21 hours
        old. ``processed_at`` was unindexed, so the statement planned a Seq Scan
        over the whole table and tried to delete a million rows in one
        transaction; whatever went wrong then went into the bare except.

        The cost of that lands on the hot path: every consumed event checks
        ``processed_events`` for deduplication, so a table that grows without
        bound makes every event handler in the service slower.
        """
        self._sweep(
            "outbox",
    ...
    def _sweep(self, table: str, batched_delete: str) -> None:
        """Run one batched retention sweep.

        Uses ``ctid IN (SELECT ... LIMIT n)`` because a plain ``DELETE ... LIMIT``
        is not valid in Postgres, and the subquery keeps the row set bounded
        whether or not the retention column happens to be indexed.

        The subquery takes ``FOR UPDATE SKIP LOCKED`` so that concurrent
        sweepers claim disjoint rows. This service runs four uvicorn workers,
        each with its own relay thread, and without it the subquery picks an
        arbitrary row set, so two sweeps lock the same rows in opposite orders
        and deadlock: the deployed cluster logged 76 deadlocks in one window,
        every one of them two of these statements waiting on each other, and
        the loser's whole batch rolled back. Skipping a locked row costs
        nothing, because the sweeper holding it is deleting it.
        """
        removed = 0
        try:
            with self._pool.connection() as conn:
                for _ in range(self._CLEANUP_MAX_BATCHES):
                    cursor = conn.execute(batched_delete, (self._CLEANUP_BATCH_SIZE,))
                    affected = cursor.rowcount if cursor.rowcount is not None and cursor.rowcount >= 0 else 0
                    removed += affected
                    conn.commit()
                    if affected < self._CLEANUP_BATCH_SIZE:
                        break
                else:
                    # Still behind after a full budget: the table is growing faster
                    # than the sweep drains it, which is how the million-row backlog
                    # accumulated in the first place.
                    LOGGER.warning(
                        "retention sweep for %s removed %d rows and hit its batch budget; "
                        "the table is still above retention. If this repeats, the retention "
                        "column likely needs an index.",
                        table,
                        removed,
                    )
        except Exception as exc:
            # Logged, not swallowed. The silent version of this except is why
            # nobody noticed the sweep had stopped for 21 hours.
            LOGGER.warning(
                "retention sweep for %s failed after removing %d rows: %s: %s",
                table,
                removed,
                type(exc).__name__,
                exc,
