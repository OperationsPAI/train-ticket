...
public class OrderServiceImpl implements OrderService {
  ...
  public Response getSoldTickets(Seat seatRequest, HttpHeaders headers) {
    ArrayList<Order> list =
        orderRepository.findByTravelDateAndTrainNumber(
            seatRequest.getTravelDate(), seatRequest.getTrainNumber());
    if (list != null && !list.isEmpty()) {
      Set ticketSet = new HashSet();
      for (Order tempOrder : list) {
        ticketSet.add(
            new Ticket(tempOrder.getSeatNumber(), tempOrder.getFrom(), tempOrder.getTo()));
      }
      LeftTicketInfo leftTicketInfo = new LeftTicketInfo();
      leftTicketInfo.setSoldTickets(ticketSet);
      OrderServiceImpl.LOGGER.info(
          "[getSoldTickets][Left ticket info][info is: {}]", leftTicketInfo.toString());
      return new Response<>(1, success, leftTicketInfo);
    } else {
      OrderServiceImpl.LOGGER.warn(
          "[getSoldTickets][Seat][Left ticket info is empty][seat from date: {}, train number: {}]",
          seatRequest.getTravelDate(),
          seatRequest.getTrainNumber()); // warn级别，获取资源但资源为空
  ...
  public Response findOrderById(String id, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      OrderServiceImpl.LOGGER.warn(
          "[findOrderById][Find Order By Id Fail][No content][id: {}] ", id); // 获取资源但资源为空
      return new Response<>(0, "No Content by this id", null);
    } else {
      Order order = op.get();
      OrderServiceImpl.LOGGER.warn(
          "[findOrderById][Find Order By Id Success][id: {}] ", id); // 获取资源但资源为空
  ...
  public Response create(Order order, HttpHeaders headers) {
    ...
    if (exists) {
      OrderServiceImpl.LOGGER.error(
          "[create][Order Create Fail][Order already exists][OrderId: {}]", order.getId());
      return new Response<>(0, "Order already exist", null);
    } else {
      order.setId(UUID.randomUUID().toString());
      order = orderRepository.save(order);
      OrderServiceImpl.LOGGER.info(
          "[create][Order Create Success][Order Price][OrderId:{} , Price: {}]",
          order.getId(),
          order.getPrice());
      return new Response<>(1, success, order);
    }
  ...
  private boolean checkDuplicateOrder(Order order) {
    ...
    return orderRepository
        .findByAccountIdAndTrainNumberAndTravelDate(
  ...
  public Response alterOrder(OrderAlterInfo oai, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      OrderServiceImpl.LOGGER.error(
          "[alterOrder][Alter Order Fail][Order do not exist][OrderId: {}]", oldOrderId);
    ...
    if (cor.getStatus() == 1) {
      OrderServiceImpl.LOGGER.info(
          "[alterOrder][Alter Order Success][newOrderId: {}]", newOrder.getId());
      return new Response<>(1, success, newOrder);
    } else {
      OrderServiceImpl.LOGGER.error(
          "[alterOrder][Alter Order Fail][Create new order fail][newOrderId: {}]",
          newOrder.getId());
  ...
  @Override
  public Response<ArrayList<Order>> queryOrders(
      OrderInfo qi, String accountId, HttpHeaders headers) {
    ...
    if (qi.isEnableStateQuery() || qi.isEnableBoughtDateQuery() || qi.isEnableTravelDateQuery()) {
      ...
      Date boughtDateStart =
          qi.isEnableBoughtDateQuery() ? StringUtils.String2Date(qi.getBoughtDateStart()) : null;
      Date boughtDateEnd =
          qi.isEnableBoughtDateQuery() ? StringUtils.String2Date(qi.getBoughtDateEnd()) : null;
      Date travelDateStart =
          qi.isEnableTravelDateQuery() ? StringUtils.String2Date(qi.getBoughtDateStart()) : null;
      Date travelDateEnd =
          qi.isEnableTravelDateQuery() ? StringUtils.String2Date(qi.getTravelDateEnd()) : null;
  ...
  private ArrayList<Order> findOrdersWithFilters(
      String accountId,
      Integer state,
      Date boughtDateStart,
      Date boughtDateEnd,
  ...
  public List<String> queryForStationId(List<String> ids, HttpHeaders headers) {
    ...
    ResponseEntity<Response<List<String>>> re =
        restTemplate.exchange(
            stationServiceUrl + "/api/v1/stationservice/stations/namelist",
            HttpMethod.POST,
            requestEntity,
            new ParameterizedTypeReference<Response<List<String>>>() {});
    OrderServiceImpl.LOGGER.info(
        "[queryForStationId][Station Name List][Name List is: {}]", re.getBody().toString());
  ...
  public Response saveChanges(Order order, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      OrderServiceImpl.LOGGER.error(
          "[saveChanges][Modify Order Fail][Order not found][OrderId: {}]", order.getId());
      ...
      OrderServiceImpl.LOGGER.info(
          "[saveChanges][Modify Order Success][OrderId: {}]", order.getId());
  ...
  public Response cancelOrder(String accountId, String orderId, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      OrderServiceImpl.LOGGER.error(
          "[cancelOrder][Cancel Order Fail][Order not found][OrderId: {}]", orderId);
      return new Response<>(0, orderNotFound, null);
    } else {
      Order oldOrder = op.get();
      oldOrder.setStatus(OrderStatus.CANCEL.getCode());
  ...
  public Response queryAlreadySoldOrders(Date travelDate, String trainNumber, HttpHeaders headers) {
    ...
    OrderServiceImpl.LOGGER.info(
        "[queryAlreadySoldOrders][Calculate Sold Ticket][Get Orders Number: {}]", orders.size());

    // Use stream grouping for more efficient counting
    Map<Integer, Long> seatClassCounts =
        orders.stream()
    ...
    seatClassCounts.forEach(
        (seatClass, count) -> {
          switch (seatClass) {
            case 0:
              cstr.setNoSeat(cstr.getNoSeat() + count.intValue());
              break;
            case 1:
              cstr.setBusinessSeat(cstr.getBusinessSeat() + count.intValue());
              break;
            case 2:
              cstr.setFirstClassSeat(cstr.getFirstClassSeat() + count.intValue());
              break;
            case 3:
              cstr.setSecondClassSeat(cstr.getSecondClassSeat() + count.intValue());
              break;
            case 4:
              cstr.setHardSeat(cstr.getHardSeat() + count.intValue());
              break;
            case 5:
              cstr.setSoftSeat(cstr.getSoftSeat() + count.intValue());
              break;
            case 6:
              cstr.setHardBed(cstr.getHardBed() + count.intValue());
              break;
            case 7:
              cstr.setSoftBed(cstr.getSoftBed() + count.intValue());
              break;
            case 8:
              cstr.setHighSoftBed(cstr.getHighSoftBed() + count.intValue());
              break;
            default:
              OrderServiceImpl.LOGGER.info(
                  "[queryAlreadySoldOrders][Calculate Sold Tickets][Unknown seat class: {}]",
                  seatClass);
          }
        });
  ...
  public Response getAllOrders(HttpHeaders headers) {
    ...
    if (orderPage != null && orderPage.hasContent()) {
      ...
      OrderServiceImpl.LOGGER.info(
          "[getAllOrders][Find orders with pagination][page size:{}, total:{}]",
          orders.size(),
  ...
  public Response modifyOrder(String orderId, int status, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      OrderServiceImpl.LOGGER.error(
          "[modifyOrder][Modify order Fail][Order not found][OrderId: {}]", orderId);
      return new Response<>(0, orderNotFound, null);
    } else {
      Order order = op.get();
  ...
  public Response getOrderPrice(String orderId, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      OrderServiceImpl.LOGGER.error(
          "[getOrderPrice][Get order price Fail][Order not found][OrderId: {}]", orderId);
      return new Response<>(0, orderNotFound, "-1.0");
    } else {
      Order order = op.get();
      OrderServiceImpl.LOGGER.info(
          "[getOrderPrice][Get Order Price Success][OrderId: {} , Price: {}]",
          orderId,
          order.getPrice());
  ...
  public Response payOrder(String orderId, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      OrderServiceImpl.LOGGER.error(
          "[payOrder][Pay order Fail][Order not found][OrderId: {}]", orderId);
      return new Response<>(0, orderNotFound, null);
    } else {
      Order order = op.get();
  ...
  public Response getOrderById(String orderId, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      OrderServiceImpl.LOGGER.warn(
          "[getOrderById][Get Order By ID Fail][Order not found][OrderId: {}]", orderId);
      return new Response<>(0, orderNotFound, null);
    } else {
      Order order = op.get();
  ...
  public void initOrder(Order order, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      ...
      OrderServiceImpl.LOGGER.info(
          "[initOrder][Init Order Success][OrderId: {}]", newOrder.getId());
    } else {
      Order orderTemp = op.get();
      OrderServiceImpl.LOGGER.error(
          "[initOrder][Init Order Fail][Order Already Exists][OrderId: {}]", order.getId());
    }
  }
  ...
  public Response deleteOrder(String orderId, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      OrderServiceImpl.LOGGER.error(
          "[deleteOrder][Delete order Fail][Order not found][OrderId: {}]", orderId);
  ...
  public Response addNewOrder(Order order, HttpHeaders headers) {
    ...
    if (exists) {
      OrderServiceImpl.LOGGER.error(
          "[addNewOrder][Admin Add Order Fail][Order already exists][OrderId: {}]", order.getId());
      return new Response<>(0, "Order already exist", null);
    } else {
      order.setId(UUID.randomUUID().toString());
      Order newOrder = orderRepository.save(order);
      OrderServiceImpl.LOGGER.info(
          "[addNewOrder][Admin Add Order Success][OrderId: {} , Price: {}]",
          newOrder.getId(),
          order.getPrice());
  ...
  public Response updateOrder(Order order, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      OrderServiceImpl.LOGGER.error(
          "[updateOrder][Admin Update Order Fail][Order not found][OrderId: {}]", order.getId());
      ...
      orderRepository.save(oldOrder);
      OrderServiceImpl.LOGGER.info(
          "[updateOrder][Admin Update Order Success][OrderId: {}]", order.getId());
