...
public class ContactsServiceImpl implements ContactsService {
  ...
  public Response findContactsById(String id, HttpHeaders headers) {
    ...
    if (contacts != null) {
      ...
      LOGGER.error(
          "[findContactsById][contactsRepository.findById][No contacts according to"
  ...
  public Response findContactsByAccountId(String accountId, HttpHeaders headers) {
    ...
    ContactsServiceImpl.LOGGER.info(
        "[findContactsByAccountId][Query Contacts][Result Size: {}]", arr.size());
  ...
  public Response createContacts(Contacts contacts, HttpHeaders headers) {
    Contacts contactsTemp =
        contactsRepository.findByAccountIdAndDocumentTypeAndDocumentType(
            contacts.getAccountId(), contacts.getDocumentNumber(), contacts.getDocumentType());
    if (contactsTemp != null) {
      ContactsServiceImpl.LOGGER.warn(
          "[createContacts][Init Contacts, Already Exists][Id: {}]", contacts.getId());
  ...
  public Response create(Contacts addContacts, HttpHeaders headers) {
    ...
    if (c != null) {
      ContactsServiceImpl.LOGGER.warn(
          "[Contacts-Add&Delete-Service.create][AddContacts][Fail.Contacts already"
              + " exists][contactId: {}]",
          addContacts.getId());
  ...
  public Response delete(String contactsId, HttpHeaders headers) {
    ...
    if (contacts == null) {
      ...
      ContactsServiceImpl.LOGGER.error(
          "[Contacts-Add&Delete-Service][DeleteContacts][Fail.Reason not clear][contactsId: {}]",
          contactsId);
  ...
  public Response modify(Contacts contacts, HttpHeaders headers) {
    ...
    if (oldContacts == null) {
      ContactsServiceImpl.LOGGER.error(
          "[Contacts-Modify-Service.modify][ModifyContacts][Fail.Contacts not found][contactId:"
              + " {}]",
          contacts.getId());
  ...
  public Response getAllContacts(HttpHeaders headers) {
    ...
    if (contacts != null && !contacts.isEmpty()) {
      ...
      LOGGER.error(
          "[getAllContacts][contactsRepository.findAll][Get all contacts error][message: {}]",
          "No content");
