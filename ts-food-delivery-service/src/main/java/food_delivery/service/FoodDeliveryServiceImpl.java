...
public class FoodDeliveryServiceImpl implements FoodDeliveryService {
  ...
  @Autowired FoodDeliveryOrderRepository foodDeliveryOrderRepository;
  ...
  public Response createFoodDeliveryOrder(FoodDeliveryOrder fd, HttpHeaders headers) {
    ...
    String stationFoodServiceUrl = getServiceUrl("ts-station-food-service");
    //        stationFoodServiceUrl = "http://ts-station-food-service"; // 测试
    ResponseEntity<Response<StationFoodStoreInfo>> getStationFoodStore =
        restTemplate.exchange(
            stationFoodServiceUrl
                + "/api/v1/stationfoodservice/stationfoodstores/bystoreid/"
                + stationFoodStoreId,
            HttpMethod.GET,
    ...
    Map<String, Double> foodPrice =
        storeFoodList.stream().collect(Collectors.toMap(Food::getFoodName, Food::getPrice));
    ...
    for (Food food : orderFoodList) {
      ...
      if (fee == null) {
        LOGGER.error(
            "{}:{} have no such food: {}",
            stationFoodStoreId,
            stationFoodStoreInfo.getStoreName(),
            food.getFoodName());
  ...
  public Response getFoodDeliveryOrderByStoreId(String storeId, HttpHeaders headers) {
    List<FoodDeliveryOrder> foodDeliveryOrders =
        foodDeliveryOrderRepository.findByStationFoodStoreId(storeId);
    if (foodDeliveryOrders == null) {
      LOGGER.error("[getAllFoodDeliveryOrders] Food delivery orders query error");
      return new Response<>(0, "food delivery orders query error", storeId);
    } else {
      LOGGER.info(
          "[getAllFoodDeliveryOrders] Get food delivery orders by storeId {} success", storeId);
      return new Response<>(1, "Get success", foodDeliveryOrders);
    }
  }
  ...
  public Response updateDeliveryTime(DeliveryInfo deliveryInfo, HttpHeaders headers) {
    ...
    if (t == null) {
      ...
      foodDeliveryOrderRepository.save(t);
      LOGGER.info(
          "[updateDeliveryTime] update deliveryTime success. id:{}, deliveryTime:{}",
          id,
          deliveryTime);
