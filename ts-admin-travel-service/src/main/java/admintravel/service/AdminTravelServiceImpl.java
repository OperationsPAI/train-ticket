...
public class AdminTravelServiceImpl implements AdminTravelService {
  ...
  public Response getAllTravels(HttpHeaders headers) {
    ...
    ResponseEntity<Response<ArrayList<AdminTrip>>> re =
        restTemplate.exchange(
            travelServiceUrl + "/api/v1/travelservice/admin_trip",
            HttpMethod.GET,
            requestEntity,
    ...
    if (result.getStatus() == 1) {
      ...
      AdminTravelServiceImpl.LOGGER.info(
          "[getAllTravels][Get Travel From ts-travel-service successfully!]");
      trips.addAll(adminTrips);
    } else {
      AdminTravelServiceImpl.LOGGER.error(
          "[getAllTravels][receive response][Get Travel From ts-travel-service fail!]");
    }

    HttpEntity requestEntity2 = new HttpEntity(headers);
    String travel2ServiceUrl = getServiceUrl("ts-travel2-service");
    ResponseEntity<Response<ArrayList<AdminTrip>>> re2 =
        restTemplate.exchange(
            travel2ServiceUrl + "/api/v1/travel2service/admin_trip",
            HttpMethod.GET,
            requestEntity2,
            new ParameterizedTypeReference<Response<ArrayList<AdminTrip>>>() {});
    result = re2.getBody();
    ...
    if (result.getStatus() == 1) {
      AdminTravelServiceImpl.LOGGER.info(
          "[getAllTravels][Get Travel From ts-travel2-service successfully!]");
      ArrayList<AdminTrip> adminTrips = result.getData();
      trips.addAll(adminTrips);
    } else {
      AdminTravelServiceImpl.LOGGER.error(
          "[getAllTravels][receive response][Get Travel From ts-travel2-service fail!]");
    }
  ...
  public Response addTravel(TravelInfo request, HttpHeaders headers) {
    ...
    if (tripId.charAt(0) == 'G' || tripId.charAt(0) == 'D') {
      requestUrl = travelServiceUrl + "/api/v1/travelservice/trips";
    } else {
      requestUrl = travel2ServiceUrl + "/api/v1/travel2service/trips";
    }
    HttpEntity requestEntity = new HttpEntity(request, headers);
    ResponseEntity<Response> re =
        restTemplate.exchange(requestUrl, HttpMethod.POST, requestEntity, Response.class);
    result = re.getBody();

    if (result.getStatus() == 1) {
      if (AdminTravelServiceImpl.LOGGER.isInfoEnabled()) {
        AdminTravelServiceImpl.LOGGER.info("[addTravel][Admin add new travel][success]");
      }
      return new Response<>(1, "[Admin add new travel]", null);
    } else {
      if (AdminTravelServiceImpl.LOGGER.isErrorEnabled()) {
        AdminTravelServiceImpl.LOGGER.error(
            "[addTravel][receive response][Admin add new travel failed][trip id: {}]",
            request.getTripId());
      }
  ...
  public Response updateTravel(TravelInfo request, HttpHeaders headers) {
    ...
    HttpEntity requestEntity = new HttpEntity(request, headers);
    ResponseEntity<Response> re =
        restTemplate.exchange(requestUrl, HttpMethod.PUT, requestEntity, Response.class);
  ...
  public Response deleteTravel(String tripId, HttpHeaders headers) {
    ...
    String travel2ServiceUrl = getServiceUrl("ts-travel2-service");
    if (tripId.charAt(0) == 'G' || tripId.charAt(0) == 'D') {
      requestUtl = travelServiceUrl + "/api/v1/travelservice/trips/" + tripId;
    } else {
      requestUtl = travel2ServiceUrl + "/api/v1/travel2service/trips/" + tripId;
    }
    HttpEntity requestEntity = new HttpEntity(headers);
    ResponseEntity<Response> re =
        restTemplate.exchange(requestUtl, HttpMethod.DELETE, requestEntity, Response.class);

    result = re.getBody();
    if (result.getStatus() != 1) {
      if (AdminTravelServiceImpl.LOGGER.isErrorEnabled()) {
        AdminTravelServiceImpl.LOGGER.error(
            "[deleteTravel][receive response][Admin delete travel failed][trip id: {}]", tripId);
      }
    ...
    if (AdminTravelServiceImpl.LOGGER.isInfoEnabled()) {
      AdminTravelServiceImpl.LOGGER.info(
          "[deleteTravel][Admin delete travel success][trip id: {}]", tripId);
    }
    return result;
  }
  ...
  private Route getRouteByRouteId(String routeId, HttpHeaders headers) {
    if (AdminTravelServiceImpl.LOGGER.isInfoEnabled()) {
      AdminTravelServiceImpl.LOGGER.info(
          "[getRouteByRouteId][Get Route By Id][Route ID：{}]", routeId);
    }
    HttpEntity requestEntity = new HttpEntity(headers);
