...
public class RebookServiceImpl implements RebookService {
  ...
  public Response rebook(RebookInfo info, HttpHeaders httpHeaders) {
    ...
    if (queryOrderResult.getStatus() == 1) {
      if (queryOrderResult.getData().getStatus() != 1) {
        RebookServiceImpl.LOGGER.warn(
            "[rebook][Rebook warn][Order not suitable to rebook][OrderId: {}]", info.getOrderId());
        return new Response<>(0, "you order not suitable to rebook!", null);
      }
    } else {
      RebookServiceImpl.LOGGER.warn(
          "[rebook][Rebook warn][Order not found][OrderId: {}]", info.getOrderId());
    ...
    if (status == OrderStatus.NOTPAID.getCode()) {
      RebookServiceImpl.LOGGER.warn(
          "[rebook][Rebook warn][Order not paid][OrderId: {}]", info.getOrderId());
      ...
      RebookServiceImpl.LOGGER.warn(
          "[rebook][Rebook warn][Order can't change twice][OrderId: {}]", info.getOrderId());
      return new Response<>(
          0, "You have already changed your ticket and you can only change one time.", null);
    } else if (status == OrderStatus.COLLECTED.getCode()) {
      RebookServiceImpl.LOGGER.warn(
          "[rebook][Rebook warn][Order already collected][OrderId: {}]", info.getOrderId());
      return new Response<>(
          0, "You have already collected your ticket and you can change it now.", null);
    } else {
      RebookServiceImpl.LOGGER.warn(
          "[rebook][Rebook warn][Order can't change][OrderId: {}]", info.getOrderId());
      return new Response<>(0, "You can't change your ticket.", null);
    }

    // Check the current time and the bus time of the old order, and judge whether the ticket can be
    // changed according to the time. The ticket cannot be changed after two hours.
    if (!checkTime(order.getTravelDate(), order.getTravelTime())) {
      RebookServiceImpl.LOGGER.warn(
          "[rebook][Rebook warn][Order beyond change time][OrderId: {}]", info.getOrderId());
      return new Response<>(
          0,
          "You can only change the ticket before the train start or within 2 hours after the train"
              + " start.",
          null);
    }

    // The departure and destination cannot be changed, only the train number, seat and time can be
    // changed
    ...
    if (gtdr.getStatus() == 0) {
      RebookServiceImpl.LOGGER.warn(
          "[rebook][Rebook warn][Trip detail not found][OrderId: {}]", info.getOrderId());
      ...
      if (info.getSeatType() == SeatClass.FIRSTCLASS.getCode()) {
        if (tripResponse.getConfortClass() <= 0) {
          RebookServiceImpl.LOGGER.warn(
              "[rebook][Rebook warn][Seat Not Enough][OrderId: {},SeatType: {}]",
              info.getOrderId(),
              info.getSeatType());
    ...
    if (priceOld.compareTo(priceNew) > 0) {
      ...
      if (!drawBackMoney(info.getLoginId(), difference, httpHeaders)) {
        RebookServiceImpl.LOGGER.warn(
            "[rebook][Rebook warn][Can't draw back the difference money][OrderId: {},LoginId:"
                + " {},difference: {}]",
            info.getOrderId(),
            info.getLoginId(),
            difference);
  ...
  public Response payDifference(RebookInfo info, HttpHeaders httpHeaders) {
    ...
    if (payDifferentMoney(
        info.getOrderId(),
        info.getTripId(),
        info.getLoginId(),
        priceNew.subtract(priceOld).toString(),
        httpHeaders)) {
      return updateOrder(order, info, gtdr, ticketPrice, httpHeaders);
    } else {
      RebookServiceImpl.LOGGER.warn(
          "[payDifference][Pay difference warn][Can't pay the difference money][OrderId:"
              + " {},LoginId: {},TripId: {}]",
          info.getOrderId(),
          info.getLoginId(),
          info.getTripId());
  ...
  private Response updateOrder(
    ...
    // If the original order and the new order are located in the high-speed train and other orders
    // respectively, the original order should be deleted and created on the other side with a new
    // id.
    if ((tripGD(oldTripId) && tripGD(info.getTripId()))
        || (!tripGD(oldTripId) && !tripGD(info.getTripId()))) {

      Response changeOrderResult = updateOrder(order, info.getTripId(), httpHeaders);
      if (changeOrderResult.getStatus() == 1) {
        return new Response<>(1, "Success!", order);
      } else {
        RebookServiceImpl.LOGGER.error(
            "[updateOrder][Update order error][OrderId: {},TripId: {}]",
            info.getOrderId(),
            info.getTripId());
  ...
  public Ticket dipatchSeat(
      String date,
      String tripId,
      String startStationId,
      String endStataionId,
      int seatType,
      int tatalNum,
    ...
    String seatServiceUrl = getServiceUrl("ts-seat-service");
    ResponseEntity<Response<Ticket>> reTicket =
        restTemplate.exchange(
            seatServiceUrl + "/api/v1/seatservice/seats",
            HttpMethod.POST,
            requestEntityTicket,
        ...
        if (calDateA.get(Calendar.DAY_OF_MONTH) > calDateB.get(Calendar.DAY_OF_MONTH)) {
          ...
          if (calDateA.get(Calendar.HOUR_OF_DAY) > calDateC.get(Calendar.HOUR_OF_DAY) + 2) {
            result = false;
              ...
              && calDateA.get(Calendar.MINUTE) > calDateC.get(Calendar.MINUTE)) {
            result = false;
          }
        }
      }
  ...
  private Response<TripAllDetail> getTripAllDetailInformation(
      TripAllDetailInfo gtdi, String tripId, HttpHeaders httpHeaders) {
    Response<TripAllDetail> gtdr;
    String requestUrl = "";
    String travelServiceUrl = getServiceUrl("ts-travel-service");
    String travel2ServiceUrl = getServiceUrl("ts-travel2-service");
    if (tripId.startsWith("G") || tripId.startsWith("D")) {
      requestUrl = travelServiceUrl + "/api/v1/travelservice/trip_detail";
      // ts-travel-service:12346/travel/getTripAllDetailInfo
    } else {
      requestUrl = travel2ServiceUrl + "/api/v1/travel2service/trip_detail";
    ...
    ResponseEntity<Response<TripAllDetail>> reGetTripAllDetailResult =
        restTemplate.exchange(
            requestUrl,
            HttpMethod.POST,
  ...
  private Response createOrder(Order order, String tripId, HttpHeaders httpHeaders) {
    ...
    if (tripId.startsWith("G") || tripId.startsWith("D")) {
      ...
      requestUrl = orderServiceUrl + "/api/v1/orderservice/order";
    } else {
      // ts-order-other-service:12032/orderOther/create
      requestUrl = orderOtherServiceUrl + "/api/v1/orderOtherService/orderOther";
    }
    HttpHeaders newHeaders = getAuthorizationHeadersFrom(httpHeaders);
    HttpEntity requestCreateOrder = new HttpEntity(order, newHeaders);
    ResponseEntity<Response> reCreateOrder =
        restTemplate.exchange(requestUrl, HttpMethod.POST, requestCreateOrder, Response.class);
  ...
  private Response updateOrder(Order info, String tripId, HttpHeaders httpHeaders) {
    ...
    String orderOtherServiceUrl = getServiceUrl("ts-order-other-service");
    if (tripGD(tripId)) {
      requestOrderUtl = orderServiceUrl + "/api/v1/orderservice/order";
    } else {
      requestOrderUtl = orderOtherServiceUrl + "/api/v1/orderOtherService/orderOther";
    }
    HttpHeaders newHeaders = getAuthorizationHeadersFrom(httpHeaders);
    HttpEntity requestUpdateOrder = new HttpEntity(info, newHeaders);
    ResponseEntity<Response> reUpdateOrder =
        restTemplate.exchange(requestOrderUtl, HttpMethod.PUT, requestUpdateOrder, Response.class);
  ...
  private Response deleteOrder(String orderId, String tripId, HttpHeaders httpHeaders) {
    ...
    String orderOtherServiceUrl = getServiceUrl("ts-order-other-service");
    if (tripGD(tripId)) {
      requestUrl = orderServiceUrl + "/api/v1/orderservice/order/" + orderId;
    } else {
      requestUrl = orderOtherServiceUrl + "/api/v1/orderOtherService/orderOther/" + orderId;
    }
    HttpHeaders newHeaders = getAuthorizationHeadersFrom(httpHeaders);
    HttpEntity requestDeleteOrder = new HttpEntity(newHeaders);
    ResponseEntity<Response> reDeleteOrder =
        restTemplate.exchange(requestUrl, HttpMethod.POST, requestDeleteOrder, Response.class);
  ...
  private Response<Order> getOrderByRebookInfo(RebookInfo info, HttpHeaders httpHeaders) {
    ...
    // Change can only be changed once, check the status of the order to determine whether it has
    // been changed
    String requestUrl = "";
    String orderServiceUrl = getServiceUrl("ts-order-service");
    String orderOtherServiceUrl = getServiceUrl("ts-order-other-service");
    if (info.getOldTripId().startsWith("G") || info.getOldTripId().startsWith("D")) {
      requestUrl = orderServiceUrl + "/api/v1/orderservice/order/" + info.getOrderId();
    } else {
      requestUrl =
          orderOtherServiceUrl + "/api/v1/orderOtherService/orderOther/" + info.getOrderId();
    }
    HttpHeaders newHeaders = getAuthorizationHeadersFrom(httpHeaders);
    HttpEntity requestEntityGetOrderByRebookInfo = new HttpEntity(newHeaders);
    ResponseEntity<Response<Order>> reGetOrderByRebookInfo =
        restTemplate.exchange(
            requestUrl,
            HttpMethod.GET,
  ...
  public TrainType queryTrainTypeByName(String trainTypeName, HttpHeaders headers) {
    ...
    String trainServiceUrl = getServiceUrl("ts-train-service");
    ResponseEntity<Response> re =
        restTemplate.exchange(
            trainServiceUrl + "/api/v1/trainservice/trains/byName/" + trainTypeName,
            HttpMethod.GET,
            requestEntity,
            Response.class);
  ...
  private Route getRouteByRouteId(String routeId, HttpHeaders headers) {
    HttpEntity requestEntity = new HttpEntity(headers);
    String routeServiceUrl = getServiceUrl("ts-route-service");
    ResponseEntity<Response> re =
        restTemplate.exchange(
            routeServiceUrl + "/api/v1/routeservice/routes/" + routeId,
            HttpMethod.GET,
            requestEntity,
            Response.class);
  ...
  private boolean payDifferentMoney(
      String orderId, String tripId, String userId, String money, HttpHeaders httpHeaders) {
    ...
    ResponseEntity<Response> rePayDifferentMoney =
        restTemplate.exchange(
            insidePaymentServiceUrl + "/api/v1/inside_pay_service/inside_payment/difference",
            HttpMethod.POST,
  ...
  private boolean drawBackMoney(String userId, String money, HttpHeaders httpHeaders) {
    ...
    String insidePaymentServiceUrl = getServiceUrl("ts-inside-payment-service");
    ResponseEntity<Response> reDrawBackMoney =
        restTemplate.exchange(
