...
public class BasicServiceImpl implements BasicService {
  ...
  public Response queryForTravel(Travel info, HttpHeaders headers) {
    ...
    if (!startingPlaceExist || !endPlaceExist) {
      ...
      if (!startingPlaceExist)
        if (BasicServiceImpl.LOGGER.isWarnEnabled()) {
          BasicServiceImpl.LOGGER.warn(
              "[queryForTravel][Start place not exist][start place: {}]", info.getStartPlace());
        }
      if (!endPlaceExist)
        if (BasicServiceImpl.LOGGER.isWarnEnabled()) {
          BasicServiceImpl.LOGGER.warn(
              "[queryForTravel][End place not exist][end place: {}]", info.getEndPlace());
        }
    }
    ...
    if (route.getStations().contains(start)
      ...
      if (LOGGER.isInfoEnabled()) {
        LOGGER.info(
            "[queryForTravel][query start index and end index][indexStart: {} indexEnd: {}]",
            indexStart,
            indexEnd);
      }
      if (LOGGER.isInfoEnabled()) {
        LOGGER.info(
            "[queryForTravel][query stations and distances][stations: {} distances: {}]",
            route.getStations(),
            route.getDistances());
      }
    } else {
    ...
    PriceConfig priceConfig =
        queryPriceConfigByRouteIdAndTrainType(routeId, trainType.getName(), headers);
    HashMap<String, String> prices = new HashMap<>();
    try {
      if (BasicServiceImpl.LOGGER.isInfoEnabled()) {
      ...
      if (BasicServiceImpl.LOGGER.isErrorEnabled()) {
  ...
  public Response queryForTravels(List<Travel> infos, HttpHeaders headers) {
    ...
    Map<String, PriceConfig> pcMap =
        queryPriceConfigByRouteIdsAndTrainTypes(routeIdAndTypes, headers);
  ...
  public Response queryForStationId(String stationName, HttpHeaders headers) {
    BasicServiceImpl.LOGGER.info(
        "[queryForStationId][Query For Station Id][stationName: {}]", stationName);
    ...
    if (re.getBody().getStatus() != 1) {
      ...
      BasicServiceImpl.LOGGER.warn(
          "[queryForStationId][Query for stationId error][stationName: {}, message: {}]",
          stationName,
          msg);
  ...
  public Map<String, String> checkStationsExists(List<String> stationNames, HttpHeaders headers) {
    BasicServiceImpl.LOGGER.info(
        "[checkStationsExists][Check Stations Exists][stationNames: {}]", stationNames);
    HttpEntity requestEntity = new HttpEntity(stationNames, headers);
    String stationServiceUrl = getServiceUrl("ts-station-service");
    ResponseEntity<Response> re =
        restTemplate.exchange(
            stationServiceUrl + "/api/v1/stationservice/stations/idlist",
            HttpMethod.POST,
            requestEntity,
            Response.class);
  ...
  public boolean checkStationExists(String stationName, HttpHeaders headers) {
    BasicServiceImpl.LOGGER.info(
        "[checkStationExists][Check Station Exists][stationName: {}]", stationName);
    HttpEntity requestEntity = new HttpEntity(headers);
    String stationServiceUrl = getServiceUrl("ts-station-service");
    ResponseEntity<Response> re =
        restTemplate.exchange(
  ...
  public List<TrainType> queryTrainTypeByNames(List<String> trainTypeNames, HttpHeaders headers) {
    BasicServiceImpl.LOGGER.info(
        "[queryTrainTypeByNames][Query Train Type][Train Type names: {}]", trainTypeNames);
    HttpEntity requestEntity = new HttpEntity(trainTypeNames, headers);
    String trainServiceUrl = getServiceUrl("ts-train-service");
    ResponseEntity<Response> re =
        restTemplate.exchange(
            trainServiceUrl + "/api/v1/trainservice/trains/byNames",
            HttpMethod.POST,
            requestEntity,
            Response.class);
    ...
    List<TrainType> tts =
        Arrays.asList(JsonUtils.conveterObject(response.getData(), TrainType[].class));
    return tts;
  }
  ...
  public TrainType queryTrainTypeByName(String trainTypeName, HttpHeaders headers) {
    if (BasicServiceImpl.LOGGER.isInfoEnabled()) {
      BasicServiceImpl.LOGGER.info(
          "[queryTrainTypeByName][Query Train Type][Train Type name: {}]", trainTypeName);
    }
    HttpEntity requestEntity = new HttpEntity(headers);
    String trainServiceUrl = getServiceUrl("ts-train-service");
    ResponseEntity<Response> re =
  ...
  private List<Route> getRoutesByRouteIds(List<String> routeIds, HttpHeaders headers) {
    ...
    if (result.getStatus() == 0) {
      if (BasicServiceImpl.LOGGER.isWarnEnabled()) {
        BasicServiceImpl.LOGGER.warn(
            "[getRoutesByRouteIds][Get Route By Ids Failed][Fail msg: {}]", result.getMsg());
      }
      return null;
    } else {
      if (BasicServiceImpl.LOGGER.isInfoEnabled()) {
  ...
  private Route getRouteByRouteId(String routeId, HttpHeaders headers) {
    ...
    if (result.getStatus() == 0) {
      if (BasicServiceImpl.LOGGER.isWarnEnabled()) {
        BasicServiceImpl.LOGGER.warn(
            "[getRouteByRouteId][Get Route By Id Failed][Fail msg: {}]", result.getMsg());
      }
      return null;
    } else {
      if (BasicServiceImpl.LOGGER.isInfoEnabled()) {
  ...
  private PriceConfig queryPriceConfigByRouteIdAndTrainType(
      String routeId, String trainType, HttpHeaders headers) {
    if (BasicServiceImpl.LOGGER.isInfoEnabled()) {
      BasicServiceImpl.LOGGER.info(
          "[queryPriceConfigByRouteIdAndTrainType][Query For Price Config][RouteId: {} ,TrainType:"
              + " {}]",
          routeId,
          trainType);
    }
    HttpEntity requestEntity = new HttpEntity(null, headers);
    String priceServiceUrl = getServiceUrl("ts-price-service");
    ResponseEntity<Response> re =
        restTemplate.exchange(
            priceServiceUrl + "/api/v1/priceservice/prices/" + routeId + "/" + trainType,
            HttpMethod.GET,
            requestEntity,
            Response.class);
    Response result = re.getBody();
    ...
    if (BasicServiceImpl.LOGGER.isInfoEnabled()) {
      BasicServiceImpl.LOGGER.info(
          "[queryPriceConfigByRouteIdAndTrainType][Response Resutl to String][result: {}]",
          result.toString());
    }
    return JsonUtils.conveterObject(result.getData(), PriceConfig.class);
  }

  private Map<String, PriceConfig> queryPriceConfigByRouteIdsAndTrainTypes(
      List<String> routeIdsTypes, HttpHeaders headers) {
    if (BasicServiceImpl.LOGGER.isInfoEnabled()) {
      BasicServiceImpl.LOGGER.info(
          "[queryPriceConfigByRouteIdsAndTrainTypes][Query For Price Config][RouteId and TrainType:"
              + " {}]",
          routeIdsTypes);
    }
    ...
    ResponseEntity<Response> re =
        restTemplate.exchange(
            priceServiceUrl + "/api/v1/priceservice/prices/byRouteIdsAndTrainTypes",
            HttpMethod.POST,
            requestEntity,
            Response.class);
    ...
    if (result.getStatus() == 0) {
      if (BasicServiceImpl.LOGGER.isWarnEnabled()) {
        BasicServiceImpl.LOGGER.warn(
            "[queryPriceConfigByRouteIdsAndTrainTypes][Get Price Config by routeId and trainType"
                + " Failed][Fail msg: {}]",
            result.getMsg());
      }
      return null;
      ...
      try {
        pcMap =
            mapper.readValue(
                JsonUtils.object2Json(result.getData()),
                new TypeReference<Map<String, PriceConfig>>() {});
      } catch (Exception e) {
        if (BasicServiceImpl.LOGGER.isWarnEnabled()) {
          BasicServiceImpl.LOGGER.warn(
              "[queryPriceConfigByRouteIdsAndTrainTypes][Get Price Config by routeId and trainType"
      ...
      if (BasicServiceImpl.LOGGER.isInfoEnabled()) {
        BasicServiceImpl.LOGGER.info(
            "[queryPriceConfigByRouteIdsAndTrainTypes][Get Price Config by routeId and"
                + " trainType][Success][priceConfigs: {}]",
            result.getData());
      }
      return pcMap;
    }
