...
public class TravelServiceImpl implements TravelService {
  ...
  public Response getRouteByTripId(String tripId, HttpHeaders headers) {
    ...
    if (trip == null) {
      TravelServiceImpl.LOGGER.error(
          "[getRouteByTripId][Get Route By Trip ID Fail][Trip Not Found][TripId: {}]", tripId);
      ...
      if (route == null) {
        TravelServiceImpl.LOGGER.error(
            "[getRouteByTripId][Get route by Trip id error][Route not found][RouteId: {}]",
            trip.getRouteId());
        return new Response<>(
            0, "\"[Get Route By Trip ID] Route Not Found:\" + trip.getRouteId()", null);
      } else {
  ...
  public Response getTripByRoute(ArrayList<String> routeIds, HttpHeaders headers) {
    ...
    if (!tripList.isEmpty()) {
      ...
      TravelServiceImpl.LOGGER.warn(
          "[getTripByRoute][Get Trips by Route ids warn][Trips not found][RouteIdNumber: {}]",
          routeIds.size());
      return new Response<>(0, noCnontent, null);
    }
  }
  ...
  public Response create(edu.fudan.common.entity.TravelInfo info, HttpHeaders headers) {
    ...
    if (repository.findByTripId(ti) == null) {
      ...
      TravelServiceImpl.LOGGER.error(
          "[getTripByRoute][Create trip error][Trip already exists][TripId: {}]", info.getTripId());
  ...
  public Response getTripAllDetailInfo(TripAllDetailInfo gtdi, HttpHeaders headers) {
    ...
    if (trip == null) {
      ...
      TripResponse tripResponse =
          getTickets(trip, null, gtdi.getFrom(), gtdi.getTo(), gtdi.getTravelDate(), headers);
      if (tripResponse == null) {
        gtdr.setTrip(null);
        gtdr.setTripResponse(null);
        TravelServiceImpl.LOGGER.warn(
            "[getTripAllDetailInfo][Query trip error][Tickets not found][start: {},end: {}]",
            gtdi.getTo(),
            gtdi.getFrom());
  ...
  private Route getRouteByRouteId(String routeId, HttpHeaders headers) {
    ...
    if (result.getStatus() == 0) {
      TravelServiceImpl.LOGGER.error(
          "[getRouteByRouteId][Get Route By Id Fail][Route not found][RouteId: {}]", routeId);
      return null;
    } else {
