...
触发点包括 `CapacityReleased`、`InventoryAdjusted` 增加可售、`QuotaReleased`、`InventoryUnfrozen`、供应商释放回传。本上下文发布 `WaitlistTriggerRaised`，包含 `inventoryPoolId`、`stationInterval`、`seatClass`、`capacityUnit` 或容量桶数量、触发原因和版本。Waitlist 决定队列排序、资格、资金担保和是否发起 `HoldCapacityForWaitlist`。为避免普通购票和候补无序竞争，第一阶段建议配置短暂候补优先窗口，具体策略进入 Section 12。
...
|---|---|---|---|
| Open Question | high-level 文档中 `Waitlist` 有业务流和 Saga，但 context-map 尚未把 Waitlist 明确列为独立上下文；本设计假设 Waitlist 队列排序和候补单状态不归 Capacity & Availability。 | Capacity & Availability, Waitlist, Booking Orchestration, Payment | Reduce 阶段明确 Waitlist 是否作为独立核心/支撑上下文；若暂不独立，至少定义 WaitlistRequest 聚合归属。 |
| Open Question | 普通购票与候补在 `CapacityReleased` 后的优先级未定。若立即释放给普通搜索，可能破坏候补公平；若候补独占窗口过长，会降低实时销售体验。 | Capacity & Availability, Waitlist, Offer Management, Trip Planning | 定义候补优先窗口、库存释放后保留策略和超时回流规则，并配置到 `CapacityPolicy`。 |
| Open Question | 外部供应商库存权威场景下，`CapacityHoldConfirmed` 是否代表平台 Occupancy，还是必须等待 provider final confirmation 后才 Confirmed。 | Capacity & Availability, Provider Integration, Booking Orchestration, Entitlement & Ticketing | 按 `inventoryAuthority = internal/provider/mixed` 明确状态映射；provider-owned 场景下 `Confirmed` 必须有 provider confirmation evidence。 |
| Potential Conflict | `CapacityHold` 在 high-level aggregate model 中承担“某资源不能重复售卖”，但本设计引入 `InventoryPool` 和 `CapacityLedger` 作为补充聚合，需确认不会扩大边界到 Service Plan 或 Reporting。 | Capacity & Availability, Service Plan, Reporting | Reduce 阶段确认 `InventoryPool` 只保护库存写不变量，Service Plan 仍拥有计划，Reporting 仍只读分析。 |
| Open Question | 火车无座票、站票或容量桶如何与具体座位区间库存共同售卖，是否允许无座超出座席容量。 | Capacity & Availability, Service Plan, Entitlement & Ticketing, Fare & Pricing | 第一阶段将无座建模为 `capacityType=StandingBucket`，由 Service Plan/Fare & Pricing 提供上限和可售规则。 |
| Open Question | 换编组、经停变化已经存在已售区间时，是否自动重排座位、冻结所有库存，还是进入 Disruption Recovery。 | Capacity & Availability, Service Plan, Disruption Recovery, Post Sales | 第一阶段采用保守策略：影响站序或座席布局时冻结相关 Inventory，发布受影响范围，由 Disruption Recovery 决定恢复方案。 |
| Open Question | Offer 是否需要强 Hold 才能报价。当前高层设计要求 Offer 不锁库存，本设计遵循该规则；但高并发抢票体验可能需要短时 quote-reservation。 | Offer Management, Capacity & Availability, Booking Orchestration | 第一阶段 Offer 不 Hold；若未来引入 quote-reservation，必须作为独立 Hold purpose，设置极短 TTL 和配额上限。 |
| Open Question | Admin 人工调库存是否允许直接增加 SellableInventory，还是必须修改 PhysicalCapacity/Quota/Restriction 的来源事实。 | Capacity & Availability, Admin & Audit, Service Plan, Reporting | 建议只允许有原因的 `InventoryAdjustment` 追加事件，不直接改最终计数；Reduce 阶段定义审批级别。 |
...
- 与 high-level 设计冲突或未决问题已进入 Section 12 Reduce 阶段问题表。
