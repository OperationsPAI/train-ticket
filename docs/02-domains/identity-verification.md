...
| Journey Order | `POST /api/v1/journey-orders` HTTP request fields `travelerRefs`、`segmentRefs` | 下单前校验由调用方根据 Journey Order 创建请求中的旅客与行程引用发起；本域不消费新订单事件来决定是否可下单。 |
| Fare & Pricing | `POST /api/v1/fare-quotes` HTTP request fields `travelerRefs`、`channel`、`segmentRefs`、`productCode` | 报价前或报价中需要按 travelerRefs 查询可用优惠资质；本域只返回资质事实，不计算价格。 |
...
|---|---|---|
| Journey Order | Existing `POST /api/v1/journey-orders` HTTP request fields `travelerRefs`、`segmentRefs` identify verification lookup scope | Journey Order 在接受既有创建订单 HTTP 请求前检查 TravelerRef/证件/优惠资质是否可用于新交易；本域后续查询契约不在本设计中新增。 |
| Fare & Pricing | Existing `POST /api/v1/fare-quotes` HTTP request fields `travelerRefs`、`channel`、`segmentRefs`、`productCode` identify eligibility lookup scope | Fare & Pricing 可使用本域返回的 `EligibilityCertificate` 摘要选择优惠规则；价格和金额计算仍归 Fare & Pricing。 |
| Risk & Compliance | New Identity Verification domain event `PurchaseLimitFactRecorded`（以及第 7 节定义的 confirm/release/missed/failed 限购事实事件） | 风控未来消费本域按证件/自然人簇发布的限购事实；`AssessRisk` 是 risk-compliance 自己消费的命令，本域不把它列为 risk-compliance 发给本域的输入。 |

说明：上表中未标记为 New Identity Verification domain event 的 Traveler Profile、Journey Order、Fare & Pricing、Risk & Compliance 事件/端点名均来自 `docs/08-contracts/` 既有文档；本域新增读模型只在第 9 节出现，后续契约文档另行定义。
