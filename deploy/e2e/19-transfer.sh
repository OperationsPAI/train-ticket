...
# Run-scoped unique refs: fixed identifiers collide with prior runs'
# idempotent reports/connections and break repeatability.
RUN=$(uuid7 | tail -c 13)
...
req POST transfer-management /api/v1/segment-status-reports "{\"segmentRef\":\"seg-prev-prot-$RUN\",\"reportType\":\"DELAY\",\"reportedBy\":{\"actorType\":\"SYSTEM\",\"actorId\":\"ops\"},\"sourceSystem\":\"OPERATIONS\",\"sourceRecordId\":\"delay-risk-$RUN\",\"observedAt\":\"$(iso 1)\",\"estimatedArrivalAt\":\"$(iso 40)\"}"
...
req POST transfer-management /api/v1/segment-status-reports "{\"segmentRef\":\"seg-prev-prot-$RUN\",\"reportType\":\"DELAY\",\"reportedBy\":{\"actorType\":\"SYSTEM\",\"actorId\":\"ops\"},\"sourceSystem\":\"OPERATIONS\",\"sourceRecordId\":\"delay-missed-$RUN\",\"observedAt\":\"$(iso 2)\",\"estimatedArrivalAt\":\"$(iso 80)\"}"
...
# WAIT-only system cases auto-complete at open (wave-17 auto-WAIT design):
# no user selection happens; the case is already terminal.
CASE_STATUS=$(echo "$RESP" | python3 -c 'import sys,json; print(json.load(sys.stdin)["status"])')
[ "$CASE_STATUS" = "RECOVERED" ] && ok "WAIT auto-recovery completed" || bad "case status $CASE_STATUS"
sleep 2
