...
public class UserServiceImpl implements UserService {
  ...
  public Response saveUser(UserDto userDto, HttpHeaders headers) {
    ...
    if (user1 == null) {
      ...
      UserServiceImpl.LOGGER.error(
          "[saveUser][Save user error][User already exists][UserId: {}]", userDto.getUserId());
  ...
  private Response createDefaultAuthUser(AuthDto dto) {
    ...
    ResponseEntity<Response<AuthDto>> res =
  ...
  public Response findByUserName(String userName, HttpHeaders headers) {
    ...
    UserServiceImpl.LOGGER.warn(
        "[findByUserName][Get user by name warn,user is null][UserName: {}]", userName);
    return new Response<>(0, "No User", null);
  }
  ...
  public Response findByUserId(String userId, HttpHeaders headers) {
    ...
    UserServiceImpl.LOGGER.error(
        "[findByUserId][Get user by id error,user is null][UserId: {}]", userId);
    return new Response<>(0, "No User", null);
  }
  ...
  public Response deleteUser(String userId, HttpHeaders headers) {
    ...
    if (user != null) {
      ...
      UserServiceImpl.LOGGER.error(
          "[deleteUser][Delete user error][User not found][UserId: {}]", userId);
  ...
  public Response updateUser(UserDto userDto, HttpHeaders headers) {
    ...
    if (oldUser != null) {
      ...
      UserServiceImpl.LOGGER.error(
          "[updateUser][Update user error][User not found][UserId: {}]", userDto.getUserId());
      return new Response(0, "USER NOT EXISTS", null);
    }
  }
  ...
  public void deleteUserAuth(String userId, HttpHeaders headers) {
    ...
    String auth_service_url = getServiceUrl("ts-auth-service");
    String authServiceUri = auth_service_url + "/api/v1";
    restTemplate.exchange(
        authServiceUri + "/users/" + userId, HttpMethod.DELETE, httpEntity, Response.class);
