...
public class OrderOtherServiceImpl implements OrderOtherService {
  ...
  @Autowired private OrderOtherRepository orderOtherRepository;
  ...
  public Response getSoldTickets(Seat seatRequest, HttpHeaders headers) {
    ArrayList<Order> list =
        orderOtherRepository.findByTravelDateAndTrainNumber(
    ...
    if (list != null && !list.isEmpty()) {
      ...
      OrderOtherServiceImpl.LOGGER.info(
          "[getSoldTickets][Left ticket info][info is: {}]", leftTicketInfo.toString());

      return new Response<>(1, success, leftTicketInfo);
    } else {

      OrderOtherServiceImpl.LOGGER.warn(
          "[getSoldTickets][Seat][No content][seat from date: {}, train number: {}",
  ...
  public Response findOrderById(String id, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      OrderOtherServiceImpl.LOGGER.warn(
          "[findOrderById][Find Order By Id Fail][No content][id: {}]", id);
  ...
  public Response create(Order order, HttpHeaders headers) {
    ...
    if (accountOrders.contains(order)) {
      OrderOtherServiceImpl.LOGGER.error(
      ...
      OrderOtherServiceImpl.LOGGER.info(
          "[create][Order Create Success][OrderId:{},Price: {}]", order.getId(), order.getPrice());
      return new Response<>(1, success, order);
    }
  }
  ...
  public Response<ArrayList<Order>> queryOrders(
      QueryInfo qi, String accountId, HttpHeaders headers) {
    // 1.Get all orders of the user
    ArrayList<Order> list = orderOtherRepository.findByAccountId(accountId);
    OrderOtherServiceImpl.LOGGER.info(
        "[queryOrders][Step 1][Get Orders Number of Account][size: {}]", list.size());
    ...
    if (qi.isEnableStateQuery() || qi.isEnableBoughtDateQuery() || qi.isEnableTravelDateQuery()) {
      ...
      for (Order tempOrder : list) {
        ...
        if (qi.isEnableTravelDateQuery()) {
          if (travelDate.before(travelDateEnd) && travelDate.after(boughtDateStart)) {
        ...
        if (qi.isEnableBoughtDateQuery()) {
          if (boughtDate.before(boughtDateEnd) && boughtDate.after(boughtDateStart)) {
      ...
      OrderOtherServiceImpl.LOGGER.warn(
          "[queryOrders][Orders don't fit the requirement][loginId: {}]", qi.getLoginId());
  ...
  public List<String> queryForStationId(List<String> ids, HttpHeaders headers) {
    ...
    OrderOtherServiceImpl.LOGGER.info(
        "[queryForStationId][Stations name list][Name List is : {}]", re.getBody().toString());
  ...
  public Response cancelOrder(String accountId, String orderId, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      ...
      orderOtherRepository.save(oldOrder);
      OrderOtherServiceImpl.LOGGER.info(
          "[cancelOrder][Cancel Order Success][OrderId: {}]", oldOrder.getId());
      return new Response<>(1, success, oldOrder);
    }
  }
  ...
  public Response queryAlreadySoldOrders(Date travelDate, String trainNumber, HttpHeaders headers) {
    ...
    for (Order order : orders) {
      ...
      if (order.getSeatClass() == SeatClass.NONE.getCode()) {
        ...
        OrderOtherServiceImpl.LOGGER.info(
            "[queryAlreadySoldOrders][Calculate Sold Tickets][Seat class not exists][Order ID: {}]",
            order.getId());
      }
    }
  ...
  public Response getAllOrders(HttpHeaders headers) {
    ...
    if (orders == null) {
      ...
      OrderOtherServiceImpl.LOGGER.info(
          "[getAllOrders][Find all orders Success][size:{}]", orders.size());
      return new Response<>(1, success, orders);
    }
  }
  ...
  public Response modifyOrder(String orderId, int status, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      ...
      OrderOtherServiceImpl.LOGGER.info(
          "[modifyOrder][Modify order Success][OrderId: {}]", orderId);
      return new Response<>(1, success, order);
    }
  }
  ...
  public Response getOrderById(String orderId, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      ...
      Order order = op.get();
      OrderOtherServiceImpl.LOGGER.info(
          "[getOrderById][Get Order By ID Success][OrderId: {}]", orderId);
      return new Response<>(1, success, order);
    }
  }
  ...
  public Response deleteOrder(String orderId, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      ...
      OrderOtherServiceImpl.LOGGER.info(
          "[deleteOrder][Delete order Success][OrderId: {}]", orderId);
      return new Response<>(1, success, order);
    }
  }
  ...
  public Response updateOrder(Order order, HttpHeaders headers) {
    ...
    if (!op.isPresent()) {
      ...
      orderOtherRepository.save(oldOrder);
      OrderOtherServiceImpl.LOGGER.info(
          "[updateOrder][Admin Update Order Success][OrderId:{}]", oldOrder.getId());
      return new Response<>(1, success, oldOrder);
    }
  }
}
