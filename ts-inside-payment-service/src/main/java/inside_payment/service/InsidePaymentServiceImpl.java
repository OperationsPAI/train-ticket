...
import inside_payment.entity.MoneyType;
...
public class InsidePaymentServiceImpl implements InsidePaymentService {
  ...
  @Autowired public AddMoneyRepository addMoneyRepository;
  ...
  public Response pay(PaymentInfo info, HttpHeaders headers) {
    ...
    if (info.getTripId().startsWith("G") || info.getTripId().startsWith("D")) {
      requestOrderURL = orderServiceUrl + "/api/v1/orderservice/order/" + info.getOrderId();
    } else {
      requestOrderURL =
    ...
    ResponseEntity<Response<Order>> reGetOrderResults =
    ...
    if (result.getStatus() == 1) {
      ...
      if (order.getStatus() != OrderStatus.NOTPAID.getCode()) {
        InsidePaymentServiceImpl.LOGGER.warn(
            "[Inside Payment Service.pay][Order status Not allowed to Pay]");
      ...
      if (totalExpand.compareTo(money) > 0) {
        ...
        String paymentServiceUrl = getServiceUrl("ts-payment-service");
        ResponseEntity<Response> reOutsidePaySuccess =
            restTemplate.exchange(
                paymentServiceUrl + "/api/v1/paymentservice/payment",
                HttpMethod.POST,
                requestEntityOutsidePaySuccess,
                Response.class);
        Response outsidePaySuccess = reOutsidePaySuccess.getBody();

        InsidePaymentServiceImpl.LOGGER.info(
            "[Inside Payment Service.pay][outside Pay][Out pay result: {}]",
            outsidePaySuccess.toString());
      ...
      LOGGER.error(
          "[Inside Payment Service.pay][Payment failed][Order not exists][orderId: {}]",
          info.getOrderId());
  ...
  public Response createAccount(AccountInfo info, HttpHeaders headers) {
    ...
    if (list.isEmpty()) {
      ...
      LOGGER.error(
          "[createAccount][Create Account Failed][Account already Exists][userId: {}]",
          info.getUserId());
  ...
  public Response drawBack(String userId, String money, HttpHeaders headers) {
    if (addMoneyRepository.findByUserId(userId) != null) {
      ...
      LOGGER.error(
          "[drawBack][Draw Back Money Failed][addMoneyRepository.findByUserId null][userId: {}]",
          userId);
  ...
  public Response payDifference(PaymentInfo info, HttpHeaders headers) {
    ...
    if (totalExpand.compareTo(money) > 0) {
      ...
      if (outsidePaySuccess.getStatus() == 1) {
        ...
        LOGGER.error(
            "[payDifference][Pay Difference Failed][outsidePaySuccess status not 1][orderId: {}]",
            info.getOrderId());
  ...
  private Response setOrderStatus(String tripId, String orderId, HttpHeaders headers) {
    ...
    if (tripId.startsWith("G") || tripId.startsWith("D")) {
      ...
      String orderServiceUrl = getServiceUrl("ts-order-service");
      ResponseEntity<Response> reModifyOrderStatusResult =
          restTemplate.exchange(
              orderServiceUrl + "/api/v1/orderservice/order/status/" + orderId + "/" + orderStatus,
              HttpMethod.GET,
  ...
  public void initPayment(Payment payment, HttpHeaders headers) {
    ...
    if (paymentTemp == null) {
      ...
      InsidePaymentServiceImpl.LOGGER.error(
          "[initPayment][paymentTemp Already Exists][paymentId: {}, orderId: {}]",
          payment.getId(),
          payment.getOrderId());
    }
  }
