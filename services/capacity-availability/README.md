...
Postgres-gated storage checks are ignored by default so normal unit tests and `make check` do not require a running database. To run the real optimistic-concurrency and idempotency transaction gates, point `TEST_DATABASE_URL` at a disposable PostgreSQL database, apply `migrations/001_capacity_storage.sql`, then run:

```bash
TEST_DATABASE_URL=postgres://... cargo test --manifest-path platform/rust-kit/Cargo.toml -- --ignored
TEST_DATABASE_URL=postgres://... cargo test --manifest-path services/capacity-availability/Cargo.toml --features redis-impl -- --ignored
```
