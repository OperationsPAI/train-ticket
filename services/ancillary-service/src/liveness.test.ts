 ...
 *
 * ancillary-service answers `/healthz` with the service-profile body rather
 ...
 */
describe("ancillary-service liveness reflects whether the service can still do work", () => {
  ...
  it("returns 200 while everything is healthy", async () => {
    ...
    assert.equal(healthz.json().service.serviceId, "ancillary-service");
  ...
  it("stays 200 while the Redis consumer is briefly disconnected", async () => {
    let now = 1_000;
    const consumer = register("redis-consumer.ancillary-service", 300_000, () => now);
  ...
  it("fails 503 when the outbox relay is the wedged component", async () => {
    let now = 1_000;
    const relay = register("ancillary-outbox-relay", 300_000, () => now);
    relay.markHealthy();
  ...
  it("recovers to 200 if the consumer comes back after the grace period elapsed", async () => {
    let now = 1_000;
    const consumer = register("redis-consumer.ancillary-service", 60_000, () => now);
    consumer.markHealthy();
    consumer.markUnhealthy("poll failing: Connection is closed");
    now += 120_000;
  ...
  it("keeps readiness independent of liveness", async () => {
    let now = 1_000;
    const consumer = register("redis-consumer.ancillary-service", 1_000, () => now);
    consumer.markHealthy();
