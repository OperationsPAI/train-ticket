...
export class AncillaryApplicationService {
  ...
  private async publishFact(
    ...
    await this.publisher?.publish(
      createEventEnvelope({
        eventId: `ancillary-service:AncillaryFulfillmentFactRecorded:${aggregateId}:${fulfillmentFactId}`,
...
function deterministicEventId(
  ...
  return `ancillary-service:${eventType}:${aggregateId}:${aggregateVersion}`;
}
