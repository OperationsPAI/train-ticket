...
func TestWithinReusesExistingContextTransaction(t *testing.T) {
	transactor := &Transactor{}
	tx := contextDB("tx")
	called := false
	err := transactor.Within(contextWithTx(context.Background(), tx), func(ctx context.Context) error {
		called = true
		if got := transactor.DBFor(ctx); got != storage.DBTX(tx) {
			t.Fatalf("DBFor returned wrong transaction: got %v want %v", got, tx)
		}
		return nil
	})
	if err != nil {
		t.Fatalf("Within returned error: %v", err)
	}
	if !called {
		t.Fatalf("Within did not invoke callback")
	}
}
