...
def test_probes_do_not_queue_behind_the_verification_operations() -> None:
    ...
    # threads for the whole process. Every verification operation is a `def`
    # path doing blocking work, so under load the operations own all 40 and a
    ...
    for route in app.routes:
        ...
        assert asyncio.iscoroutinefunction(route.endpoint), (
            ...
            f"40-thread pool that the verification operations occupy for the "
            f"length of their blocking work, and kubelet restarts the pod"
        )
