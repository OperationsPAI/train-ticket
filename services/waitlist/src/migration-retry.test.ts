 ...
 *
 * `storage.ts`'s own `migrationsDirectory()` resolves relative to the emitted
 * file, and this package's test build lands in `test-output/src/`, so that
 * helper would point at a directory that does not exist under test. `npm test`
 * always runs from the package root, so resolve from cwd instead.
 */
...
describe("waitlist storage recovers from a migration that fails at boot", () => {
  it("becomes ready after a retry, without a process restart", async () => {
    ...
    await within(migrations.applied(), "waitlist storage never became ready after retrying");
...
describe("waitlist probes while migrations are still being retried", () => {
  const unusedRunCommand = async <T>(): Promise<T> => {
    throw new Error("no command should run against a schema that does not exist yet");
  };

  function migratingStorage(schema: SchemaDetail) {
    return { ready: async () => false, schema: () => schema, runCommand: unusedRunCommand };
  }
  ...
  it("keeps the unchanged 200 body once migrations have applied", async () => {
    const app = createApp({ storage: { ready: async () => true, schema: () => ({ schema: "applied", attempts: 4 }), runCommand: unusedRunCommand } });
  ...
  it("reports migration state alongside the existing eventConsumption field", async () => {
    // The two states are independent and both belong in the body: a retrying
    // subscriber alone keeps /readyz at 200, but retrying migrations force 503.
    const app = createApp({
