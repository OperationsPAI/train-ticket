 ...
 * `startCustomerServiceStorage` did
 *
...
describe("customer-service storage recovers from a migration that fails at boot", () => {
  it("becomes ready after a retry, without a process restart", async () => {
    ...
    await within(migrations.applied(), "customer-service storage never became ready after retrying");
...
describe("customer-service probes while migrations are still being retried", () => {
