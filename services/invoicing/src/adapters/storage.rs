use crate::utils::{array_strings, handler_error, sha256_prefixed, storage_error, string_field};
use crate::*;
...
impl PostgresInvoicingService {
    ...
    async fn persist_order_projection(
        &self,
        event_id: &str,
        order_id: &str,
        account_id: &str,
        ...
        sqlx::query("INSERT INTO invoice_eligibility_projection (order_id, account_id, status, traveler_refs, segment_refs, source_event_id) VALUES ($1,$2,'CONFIRMED',$3,$4,$5) ON CONFLICT (order_id) DO UPDATE SET account_id=EXCLUDED.account_id,status=EXCLUDED.status,traveler_refs=EXCLUDED.traveler_refs,segment_refs=EXCLUDED.segment_refs,source_event_id=EXCLUDED.source_event_id,updated_at=now()")
            .bind(order_id).bind(account_id).bind(serde_json::to_value(traveler_refs).map_err(|e| InvoicingError::Internal(e.to_string()))?).bind(serde_json::to_value(segment_refs).map_err(|e| InvoicingError::Internal(e.to_string()))?).bind(event_id)
            .execute(self.pool()).await.map_err(|e| InvoicingError::Internal(e.to_string()))?;
        Ok(())
    }

    async fn persist_amount_projection(
        &self,
        event_id: &str,
        order_id: &str,
        ...
        sqlx::query("INSERT INTO amount_basis_projection (order_id, amount_basis, source_event_id) VALUES ($1,$2,$3) ON CONFLICT (order_id) DO UPDATE SET amount_basis=EXCLUDED.amount_basis,source_event_id=EXCLUDED.source_event_id,updated_at=now()")
            .bind(order_id).bind(serde_json::to_value(amount_basis).map_err(|e| InvoicingError::Internal(e.to_string()))?).bind(event_id)
            .execute(self.pool()).await.map_err(|e| InvoicingError::Internal(e.to_string()))?;
        Ok(())
    }

    async fn event_processed(&self, event_id: &str, stream: &str) -> Result<bool, InvoicingError> {
        let row: Option<(i64,)> =
            sqlx::query_as("SELECT 1 FROM processed_events WHERE event_id = $1 AND stream = $2")
                .bind(event_id)
                .bind(stream)
    ...
    async fn mark_processed(&self, event_id: &str, stream: &str) -> Result<(), InvoicingError> {
        sqlx::query(
    ...
    pub async fn handle_subscribed_event(
        ...
        if self
            .event_processed(&event_id, &stream)
            .await
        ...
        self.inner.handle_subscribed_event(envelope).await?;
        match event_type.as_str() {
            ...
            "RevenueRecognized" | "InvoiceGenerated" => {
                if let Some(order_id) = order_id.as_deref() {
                    ...
                    {
                        self.persist_amount_projection(&event_id, order_id, &amount_basis)
                            .await
                            .map_err(handler_error)?;
                    }
                }
            }
            "PostSalesApplied" => self.persist_snapshot().await.map_err(handler_error)?,
            _ => {}
        }
        self.mark_processed(&event_id, &stream)
            .await
            .map_err(handler_error)?;
        Ok(())
    }
...
impl InvoicingApi for PostgresInvoicingService {
    async fn create_title(
        ...
        let result = self.inner.create_title(cmd, key, corr).await?;
    ...
    async fn update_title(
        ...
        let result = self.inner.update_title(id, cmd, key, corr).await?;
        self.persist_snapshot().await?;
        Ok(result)
    }
    async fn set_default_title(
        ...
        let result = self.inner.set_default_title(id, cmd, key, corr).await?;
        self.persist_snapshot().await?;
        Ok(result)
    }
    async fn deactivate_title(
        ...
        let result = self.inner.deactivate_title(id, cmd, key, corr).await?;
        self.persist_snapshot().await?;
        Ok(result)
    }
    async fn request_invoice(
        ...
        let result = self.inner.request_invoice(cmd, key, corr).await?;
        self.persist_snapshot().await?;
        Ok(result)
    }
