...
def configure_runtime_endpoints(app: FastAPI) -> None:
    ...
    # The probes are coroutines so they never queue behind the business endpoints.
    # FastAPI serves a `def` endpoint from anyio's shared 40-thread pool, and the business endpoints are `def` paths that hold a thread for the length of their blocking downstream chain.
