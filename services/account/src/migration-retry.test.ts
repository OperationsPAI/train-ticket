...
describe("account storage recovers from a migration that fails at boot", () => {
  it("becomes ready after a retry, without a process restart", async () => {
    ...
    await within(migrations.applied(), "account storage never became ready after retrying");
...
describe("account probes while migrations are still being retried", () => {
