...
def test_probes_do_not_queue_behind_the_transfer_operations() -> None:
    ...
    # threads for the whole process. Every transfer operation is a `def` path
    # doing blocking work, including the downstream calls to recovery, so under
    # load the operations own all 40 and a `def` probe waits for one to finish.
    # An `async def` probe runs on the event loop and cannot be starved by a
    # saturated pool.
    #
    ...
    for route in app.routes:
        ...
        assert asyncio.iscoroutinefunction(route.endpoint), (
            ...
            f"40-thread pool that the transfer operations occupy for the length "
