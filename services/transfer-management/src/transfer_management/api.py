...
def configure_runtime_endpoints(app: FastAPI, tracer: TraceHook | None = None, otel_tracer: RuntimeTracer | None = None) -> None:
    ...
    async def request_context_middleware(request: Request, call_next: Any):
        ...
        attrs = {"service.name": profile()["service_id"], "request_id": request_id, "correlation_id": correlation_id, "http.request.method": request.method, "http.method": request.method, "url.path": path, "http.route": path, "http.request_id": request_id, "http.correlation_id": correlation_id}
    ...
    # The probes are coroutines so they never queue behind the transfer operations.
    # FastAPI serves a `def` endpoint from anyio's shared 40-thread pool, and the transfer operations are `def` paths that hold a thread for the length of their blocking downstream chain.
    ...
    async def readyz_endpoint(response: Response) -> dict[str, str]:
        return await ready_endpoint(response)
...
def configure_transfer_routes(app: FastAPI, store: Any | None = None, idempotency_store: IdempotencyStore | None = None, downstream: Any | None = None, topology: Any | None = None) -> None:
    ...
    if callable(unit_of_work):
        @app.middleware("http")
        async def transfer_unit_of_work_middleware(request: Request, call_next: Any):
            with unit_of_work():
...
def _postgres_store_from_env(app: FastAPI) -> tuple[Any, IdempotencyStore | None]:
    ...
    app.state._tm_service_holder = holder
