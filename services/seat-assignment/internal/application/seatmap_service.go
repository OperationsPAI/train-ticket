...
func (s *Service) handleCapacityReleased(ctx context.Context, e kitmsg.EventEnvelope) error {
	...
	return s.transitionAllocationsByHold(ctx, str(p, "holdId"), e, func(a *domain.ContractSeatAllocation, at time.Time) { a.Release(at) }, "SeatAllocationReleased", "releasedAt", map[string]any{"releaseReason": "CAPACITY_RELEASED"})
}
...
func (s *Service) handleCapacityHoldExpired(ctx context.Context, e kitmsg.EventEnvelope) error {
	...
	return s.transitionAllocationsByHold(ctx, str(p, "holdId"), e, func(a *domain.ContractSeatAllocation, at time.Time) { a.Expire(at) }, "SeatAllocationExpired", "expiredAt", map[string]any{"expiredAt": expiredAt})
}

func (s *Service) transitionAllocationsByHold(ctx context.Context, holdID string, e kitmsg.EventEnvelope, apply func(*domain.ContractSeatAllocation, time.Time), eventType, timeKey string, extra map[string]any) error {
	if holdID == "" {
		return nil
	}
	...
	allocs, err := s.repo.FindSeatAllocationsByHold(ctx, holdID)
	if err != nil {
		return err
	}
