...
func TestEntitlementVoidedReleasesSeatAllocation(t *testing.T) {
	...
	allocation, err := svc.AllocateSeat(ctx, allocReq("sb-void", "tvl-void", "hold-void", &domain.SeatPreferences{AcceptStanding: true, PreferenceVersion: "pv"}))
	if err != nil {
		t.Fatal(err)
	}
	voidedAt := time.Date(2026, 8, 2, 12, 30, 0, 0, time.UTC)
	payload, _ := json.Marshal(map[string]any{"entitlementId": "ent-void", "segmentBookingId": allocation.SeatAllocationID, "seatAllocationId": allocation.SeatAllocationID, "reason": "REFUND", "voidedAt": voidedAt.Format(time.RFC3339)})
	...
	if stored.Status != domain.AllocationStatusReleased || stored.ReleasedAt == nil || !stored.ReleasedAt.Equal(voidedAt) {
		t.Fatalf("expected released allocation at void time, got %#v", stored)
	}
	if got := pub.payloads["SeatAllocationReleased"][0]["releaseReason"]; got != "VOIDED" {
		t.Fatalf("expected VOIDED release reason, got %#v", pub.payloads["SeatAllocationReleased"][0])
	}
	assertPublishedEventID(t, pub, "SeatAllocationReleased", deterministicSeatAssignmentEventID("SeatAllocationReleased", allocation.SeatAllocationID, 2))
}

func assertPublishedEventID(t *testing.T, pub *memPub, eventType, want string) {
	t.Helper()
	for _, event := range pub.events {
		if event.EventType == eventType {
			if event.EventID != want {
				t.Fatalf("%s eventId=%s want %s", eventType, event.EventID, want)
			}
			return
		}
	}
	t.Fatalf("missing published event %s in %#v", eventType, pub.types)
}
