...
public class TrainFoodServiceImpl implements TrainFoodService {
  ...
  @Autowired TrainFoodRepository trainFoodRepository;
  ...
  public TrainFood createTrainFood(TrainFood tf, HttpHeaders headers) {
    ...
    if (tfTemp != null) {
      if (tfTemp.getFoodList().equals(tf.getFoodList())) {
        TrainFoodServiceImpl.LOGGER.error(
            "[Init TrainFood] Already Exists TripId: {}", tf.getTripId());
      } else {
        tfTemp.setFoodList(tf.getFoodList());
        tf2 = trainFoodRepository.save(tfTemp);
      }
    } else {
      tf2 = trainFoodRepository.save(tf);
    }
    return tf2;
  }
