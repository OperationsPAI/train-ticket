  ...
  public Response ticketExecute(String orderId, HttpHeaders headers) {
    ...
    if (resultFromOrder.getStatus() == 1) {
      ...
      if (order.getStatus() != OrderStatus.COLLECTED.getCode()) {
        LOGGER.error(
            "[ticketExecute][getOrderByIdFromOrder][ticket execute error: {}][orderId: {}]",
            orderStatusWrong,
            orderId);
      ...
      if (resultExecute.getStatus() == 1) {
        ...
        LOGGER.error(
            "[ticketExecute][executeOrder][executeOrder error: {}][orderId: {}]",
            resultExecute.getMsg(),
            orderId);
      ...
      if (resultFromOrder.getStatus() == 1) {
        ...
        if (order.getStatus() != OrderStatus.COLLECTED.getCode()) {
          LOGGER.error(
              "[ticketExecute][getOrderByIdFromOrderOther][ticket execute error: {}][orderId: {}]",
              orderStatusWrong,
              orderId);
          return new Response<>(0, orderStatusWrong, null);
        }
        ...
        if (resultExecute.getStatus() == 1) {
          ...
          LOGGER.error(
              "[ticketExecute][executeOrderOther][executeOrderOther error: {}][orderId: {}]",
              resultExecute.getMsg(),
              orderId);
          return new Response<>(0, resultExecute.getMsg(), null);
        }
        ...
        LOGGER.error(
            "[ticketExecute][getOrderByIdFromOrderOther][ticker execute error: {}][orderId: {}]",
            "Order Not Found",
            orderId);
  ...
  public Response ticketCollect(String orderId, HttpHeaders headers) {
    ...
    if (resultFromOrder.getStatus() == 1) {
      ...
      if (order.getStatus() != OrderStatus.PAID.getCode()
        ...
        LOGGER.error(
            "[ticketCollect][getOrderByIdFromOrder][ticket collect error: {}][orderId: {}]",
            orderStatusWrong,
            orderId);
        return new Response<>(0, orderStatusWrong, null);
      }
      ...
      if (resultExecute.getStatus() == 1) {
        ...
        LOGGER.error(
            "[ticketCollect][executeOrder][ticket collect error: {}][orderId: {}]",
            resultExecute.getMsg(),
            orderId);
        return new Response<>(0, resultExecute.getMsg(), null);
      }
      ...
      if (resultFromOrder.getStatus() == 1) {
        ...
        if (order.getStatus() != OrderStatus.PAID.getCode()
            && order.getStatus() != OrderStatus.CHANGE.getCode()) {
          LOGGER.error(
              "[ticketCollect][getOrderByIdFromOrderOther][ticket collect error: {}][orderId: {}]",
              orderStatusWrong,
              orderId);
          return new Response<>(0, orderStatusWrong, null);
        }
        ...
        Response resultExecute =
            executeOrderOther(orderId, OrderStatus.COLLECTED.getCode(), headers);
        if (resultExecute.getStatus() == 1) {
          return new Response<>(1, "Success.", null);
        } else {
          LOGGER.error(
              "[ticketCollect][executeOrderOther][ticket collect error: {}][orderId: {}]",
              resultExecute.getMsg(),
              orderId);
          return new Response<>(0, resultExecute.getMsg(), null);
        }
  ...
  private Response executeOrder(String orderId, int status, HttpHeaders headers) {
    ...
    ResponseEntity<Response> re =
        restTemplate.exchange(
            orderServiceUrl + "/api/v1/orderservice/order/status/" + orderId + "/" + status,
            HttpMethod.GET,
            requestEntity,
            Response.class);
    return re.getBody();
